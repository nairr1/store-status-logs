...
status_logs = json.loads(supabase.table("StoreStatusLogs").select("*").eq("brand", "grilld").execute().json())["data"]

store_data = requests.get("https://grilld.redcatcloud.com.au/api/v1/stores").json()["data"]
