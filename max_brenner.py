...
status_logs = json.loads(supabase.table("StoreStatusLogs").select("*").eq("brand", "maxbrenner").execute().json())["data"]

store_data = requests.get("https://maxbrenner.redcatcloud.com.au/api/v1/stores").json()["data"]
